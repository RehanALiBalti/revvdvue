    ...
    <div class="">
      <form class="row" @submit.prevent="submitForm">
        ...
        <div class="col-md-6">
          ...
          <input type="number" class="form-control form-input h35px" v-model="year" placeholder="Enter year" />
        </div>
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <input type="number" class="form-control form-input h35px" v-model="milage" placeholder="Enter mileage" />
          </div>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
          ...
          <input type="number" class="form-control form-input h35px" v-model="torque" placeholder="Torque (Nm)" />
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
          ...
          <input type="text" class="form-control form-input h35px" v-model="promoCode" placeholder="Enter promo code" />
        </div>
        <div class="col-md-6">
        ...
        <div v-for="(service, index) in carServices" :key="index" class="row">

          <!-- Service Mileage -->
          <div class="col-md-6">
            <label class="form-label">Service Mileage</label>
            <input type="number" class="form-control form-input h35px" v-model="service.serviceMilage"
              placeholder="Enter service mileage" />
          </div>

          <!-- Type of Service + Plus/Minus Buttons -->
          <div class="col-md-6 d-flex align-items-end">
            <div class="flex-grow-1">
              <label class="form-label">Type of Service</label>
              <select class="form-select form-input h35px" v-model="service.serviceType">
                <option value="">Type of Service</option>
                <option v-for="stype in serviceTypes" :key="stype" :value="stype">
                  {{ stype }}
                </option>
              </select>
            </div>
            ...
            <div class="d-flex align-items-center ms-2">
              ...
              <button type="button" class="btn add-button" @click="addServiceRow()">
                +
              </button>



              <!-- Remove Button (not shown for first row) -->
              <button v-if="index > 0" type="button" class="btn ms-2 btn-danger remove-button"
          ...
          <div class="col-md-12">
            ...
            <!-- FILE INPUT WITH INDEX BINDING -->
            <input type="file" class="form-control form-input h35px d-none" :id="'file_p_' + index" multiple
              @change="handleServiceFileChange($event, index)" />
            ...
            <div class="load-more-info w-100 d-flex justify-content-start align-items-center mb-4 mx-auto">
              <div class="list-item-btn position-relative load-more-div proceed-div ">
                ...
                <!-- UPDATED LABEL WITH INDEX -->
                <label :for="'file_p_' + index" class="signin-btnli Start Engine load-more-btn proceed-btn width-set">
                  Choose
                </label>
            ...
            <template #selected-options="{ values }">
              <div class="selected-tags">
                <span v-for="(value, index) in values" :key="index" class="selected-tag">
                  {{ value }}
                  <span class="remove-tag" @click.stop="removeCategory(value)">✕</span>
                </span>
              </div>
            </template>
        ...
        <div class="row align-items-center">
          ...
          <!-- MODIFICATION PARENTS -->
          <div v-for="(parent, pIndex) in modifications" :key="pIndex" class="mt-4">

            <!-- PARENT HEADER -->
            <h3 class="form-title">
              Modification {{ pIndex + 1 }}
            ...
            <!-- CHILD MODIFICATIONS -->
            <div v-for="(child, cIndex) in parent.items" :key="cIndex" class="row g-3 mt-2">
              ...
              <div class="col-md-5 d-flex align-items-center gap-2">
                ...
                <input v-model="child.partName" class="form-control form-input h35px" type="text"
                  placeholder="Part Name" />
              </div>

              <div class="col-md-3">
                <input v-model="child.partBrand" class="form-control form-input h35px" type="text"
                  placeholder="Part Brand" />
              </div>

              <div class="col-md-4">
                <!-- <label class="form-label">Mileage Installed</label> -->

                <div class="d-flex align-items-center gap-2">

                  <!-- INPUT -->
                  <input v-model="child.milageInstalled" class="form-control form-input h35px flex-grow-1" type="text"
                    placeholder="Mileage Installed" />

                  <!-- ADD CHILD (only last row) -->
                  <button v-if="cIndex === parent.items.length - 1" type="button" class="btn add-button"
                    title="Add another modification" @click="addChild(pIndex)">
                    +
                  </button>

                  <!-- REMOVE CHILD -->
                  <button v-if="parent.items.length > 1" type="button" class="btn btn-danger remove-button"
                    title="Remove this modification" @click="removeChild(pIndex, cIndex)">
                    −
                  </button>
              ...
              <!-- <div class="col-md-4">
                <input v-model="child.milageInstalled" class="form-control form-input h35px" type="text"
                  placeholder="Mileage Installed" />
                  <div class="load-more-info d-flex gap-2">
    
                    <button v-if="cIndex === parent.items.length - 1" type="button" class="btn add-button"
                      @click="addChild(pIndex)">
                      +
                    </button>
    
    
                    <button v-if="parent.items.length > 1" type="button" class="btn ms-2 btn-danger remove-button"
                      @click="removeChild(pIndex, cIndex)">
                      −
                    </button>
              ...
              <div class="col-md-12">
                <textarea v-model="child.notes" class="form-control form-input h35px" placeholder="Notes"></textarea>
              </div>
            ...
            <div class="col-md-12">
              <div class="load-more-info w-100 d-flex justify-content-start align-items-center mb-4 mx-auto">
                <div class="list-item-btn position-relative load-more-div proceed-div mx-auto">
                  ...
                  <!-- REMOVE WHOLE MODIFICATION -->
                  <button type="button" class="signin-btnli Start Engine load-more-btn proceed-btn width-set"
                    @click="confirmRemoveParent(pIndex)">
                    Remove
                  </button>
          ...
          <!-- <div v-for="(mod, index) in modifications" :key="index" class="modification mt-2">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-12">
                <textarea v-model="mod.notes" class="form-control form-input h35px" placeholder="Notes"></textarea>
              </div>
              <div class="col-md-12">
                <div class="load-more-info w-100 d-flex justify-content-start align-items-center mb-4 mx-auto">
                  <div class="list-item-btn position-relative load-more-div proceed-div mx-auto">
            ...
            <div v-for="(mod, index) in modifications" :key="index" class="col-md-12  p-3 rounded mt-3 d-none">
              <div class="row g-2">
                <div class="col-md-3">
...
export default {
  ...
  watch: {
    ...
    selectedModCategory(newCategories) {
      ...
      // Remove unselected parents
      this.modifications = this.modifications.filter(parent =>
        newCategories.includes(parent.category)
      );

      // Add new parent categories
      newCategories.forEach(category => {
        if (!this.modifications.some(p => p.category === category)) {
  ...
  methods: {
    removeCategory(value) {
      this.selectedModCategory =
        this.selectedModCategory.filter(cat => cat !== value);
    },

    confirmRemoveParent(parentIndex) {
      Swal.fire({
        title: "Are you sure?",
        text: "This will delete all modifications under this category.",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          ...
          // ✅ STORE CATEGORY BEFORE REMOVING
          const removedCategory = this.modifications[parentIndex].category;

          // ✅ REMOVE ONLY ONCE
          this.modifications.splice(parentIndex, 1);

          // ✅ REMOVE CATEGORY FROM SELECTED LIST
          this.selectedModCategory =
            this.selectedModCategory.filter(cat => cat !== removedCategory);

          Swal.fire(
            "Deleted!",
            "The modification has been removed.",
            "success"
          );
        }
      });
    ...
    addChild(parentIndex) {
      this.modifications[parentIndex].items.push({
        partName: "",
        partBrand: "",
        milageInstalled: "",
        notes: ""
    ...
    removeChild(parentIndex, childIndex) {
      this.modifications[parentIndex].items.splice(childIndex, 1);
    },
    ...
    removeServiceRow(index) {
      this.carServices.splice(index, 1);
    },
    handleServiceFileChange(event, index) {
      const files = Array.from(event.target.files);
      this.carServices[index].files = files;
    },
...
.add-button {
  background-color: rgba(249, 95, 25, 0.3);
  border: 1px solid #f95f19;
  color: #ffffff;
}

.remove-button {
  background-color: rgba(236, 36, 36, 0.562);
  border: 1px solid #f95f19;
  color: #ffffff;
}
